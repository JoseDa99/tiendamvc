    ...
    <div class="card-body">
        <form action="<?= ROOT ?>adminUser/update/<?= $data['data']->id ?>" method="POST">
            ...
            <div class="form-group text-left">
                ...
                <input type="email" name="email" class="form-control" placeholder="Escribe el correo electrónico" required value="<?= $data['data']->email ?? '' ?>">
            </div>
            <div class="form-group text-left">
                <label for="password1">Clave de acceso: (dejar en blanco si no desea modificarla)</label>
                <input type="password" name="password1" class="form-control" placeholder="Escribe tu contraseña">
            </div>
            <div class="form-group text-left">
